...
function Search({serverUrl}) {
  ...
  useEffect(() => {
    ...
    // console.log(`${serverUrl}/api/contents/booknames`)

    axios
      .get(`${serverUrl}/api/contents/booknames`)
      .then(resp => {
  ...
  const handleSearchClick = () => {
    ...
    // Build up search URL
    let searchUrl = `${serverUrl}/api/search/`;
    ...
    <section className="container" id="bible-content-wrapper">
      ...
      <div className="row control-wrapper">
        <form onSubmit={handleSubmitSearch}>
          ...
          <SearchPhraseEntry handleSearchClick={handleSearchClick} />
        </form>
      </div>
